...
import { FilterQuery, LeanDocument, Model, AllowStringsForIds } from 'mongoose';
...
> {
  ...
  protected async find(entityFilterQuery?: FilterQuery<TSchema>): Promise<TEntity[]> {
    return (await this.entityModel.find(entityFilterQuery, {}, { lean: true })).map(
      (entityDocument) => this.entitySchemaFactory.createFromSchema(entityDocument)
    );
  }
